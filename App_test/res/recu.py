...
def calculer_duree_attente(commande_info):
    """
    Calcule la durée entre la création et le service.
    Retourne une chaîne lisible et un booléen pour savoir si c'est trop long.
    """
    try:
        date_cmd = commande_info.get('date', '')
        heure_cmd = commande_info.get('heure', '')
        served_at = commande_info.get('served_at', None)
        if served_at:
            dt_created = datetime.strptime(f"{date_cmd} {heure_cmd}", "%d/%m/%Y %H:%M")
        ...
        if minutes < 1:
            ...
            duree = f"{int(minutes)} minute{'s' if minutes > 1 else ''}"
        else:
            heures = int(minutes // 60)
            mins = int(minutes % 60)
            duree = f"{heures} heure{'s' if heures > 1 else ''} et {mins} minute{'s' if mins > 1 else ''}"
        trop_long = minutes > 15
        return duree, trop_long
    except:
        return "N/A", False
...
def generer_recu(commande_info, fichier_pdf=None):
    ...
    style_titre = ParagraphStyle('Titre', parent=styles['Title'], fontSize=14, alignment=TA_CENTER,
                                 textColor=colors.HexColor('#2c3e50'), spaceAfter=4)
    style_sous_titre = ParagraphStyle('SousTitre', parent=styles['Normal'], fontSize=9, alignment=TA_CENTER,
                                      textColor=colors.HexColor('#7f8c8d'), spaceAfter=8)
    style_info = ParagraphStyle('Info', parent=styles['Normal'], fontSize=8, alignment=TA_CENTER, spaceAfter=2)
    style_separateur = ParagraphStyle('Separateur', parent=styles['Normal'], fontSize=8, alignment=TA_CENTER,
                                      textColor=colors.HexColor('#bdc3c7'))
    style_item = ParagraphStyle('Item', parent=styles['Normal'], fontSize=9, spaceAfter=2)
    style_item_bold = ParagraphStyle('ItemBold', parent=styles['Normal'], fontSize=9, spaceAfter=2,
                                     textColor=colors.HexColor('#2c3e50'))
    style_total = ParagraphStyle('Total', parent=styles['Normal'], fontSize=12, alignment=TA_RIGHT,
                                 textColor=colors.HexColor('#27ae60'), spaceAfter=4)
    style_tva = ParagraphStyle('TVA', parent=styles['Normal'], fontSize=8, alignment=TA_RIGHT,
                               textColor=colors.HexColor('#f39c12'), spaceAfter=2)
    style_pied = ParagraphStyle('Pied', parent=styles['Normal'], fontSize=7, alignment=TA_CENTER,
                                textColor=colors.HexColor('#95a5a6'), spaceAfter=2)
    ...
    # Temps d'attente et message
    duree, trop_long = calculer_duree_attente(commande_info)
    story.append(Paragraph(f"⏱️ Temps d'attente : {duree}", style_item))
    if trop_long:
        story.append(Paragraph("🙏 Nous vous remercions de votre patience.", style_info))
        story.append(Paragraph("Nous nous excusons pour l'attente.", style_info))
    story.append(Spacer(1, 6))
...
def afficher_apercu_recu(commande_info, parent=None):
    ...
    # Temps d'attente
    duree, trop_long = calculer_duree_attente(commande_info)
    tk.Label(contenu, text=f"⏱️ Temps d'attente : {duree}", font=('Arial', 9), bg='white').pack(anchor='e', padx=10, pady=2)
    if trop_long:
        tk.Label(contenu, text="🙏 Nous vous remercions de votre patience.", font=('Arial', 9, 'italic'), fg='#e74c3c', bg='white').pack(anchor='e', padx=10)
        tk.Label(contenu, text="Nous nous excusons pour l'attente.", font=('Arial', 9, 'italic'), fg='#e74c3c', bg='white').pack(anchor='e', padx=10)
